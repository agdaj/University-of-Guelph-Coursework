* encrypt.cob
* Encrypt Input with Trithemius Cipher
* Name: Jireh Agda
...
* open the file with the fileName given with the call and read through the text and encrypt
...
* look at each character and encrypt them as necessary
...
* change shift factor by 1 upwards, resetting to 0 when shift > 25
                add 1 to shift
                if shift > 25 then
                    move zeros to shift
                end-if
...
* prints only letters or newlines from input-record after encrypting
print-letters.
    perform
